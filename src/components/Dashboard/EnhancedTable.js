...
const EnhancedTableToolbar = ({numSelected, selected,setSelected, rows, setRows}) => {
    ...
    const deleteAndLoad= async ()=> {
      await Service.deleteBulkUser(selected,tokenContext);
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        <Typography
          ...
          component="div"
        >
          {numSelected} selected (select only 1 element for editing)
        </Typography>
      ) : (
        <Typography
          sx={{ flex: '1 1 100%' }}
    ...
    {numSelected === 1 ? (
        <Tooltip title="Edit">
          <IconButton onClick={()=> {setEmail(selected[0]); setOldEmail(selected[0]); setUpdateMode(true);}}>
            <EditIcon />
          </IconButton>
        </Tooltip>)
        : null
...
const EnhancedTable= ({rows,setRows}) => {
  ...
  const { tokenContext} = useContext(UserContext);
  const { setIsAdmin} = useContext(UserIdContext);
  ...
  const handleClick = (event, email) => {
    ...
    if(newSelected.length === 1) {
      const role = rows.filter(row => row.email === newSelected[0]).map(filteredRow => ( filteredRow.role))
      setIsAdmin(role[0]==='Admin');
    }
